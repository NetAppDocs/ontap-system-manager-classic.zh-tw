...
* 如果您在定義子網路時指定閘道、則當使用該子網路建立LIF時、會自動將通往該閘道的預設路由新增至SVM。如果您不使用子網路，或定義子網路時未指定閘道，則需要使用 `route create`命令手動將路由新增至 SVM 。
...
* 您可以設定 `-force-update-lif-associations`選項的值 `true`。
+
